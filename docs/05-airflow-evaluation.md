...
- 최대 2700개 파티클, ~90개/sec spawn rate (`spawnAccumulator` 로 dt 보정 — 시연 가시성을 위해 초기 30/sec 대비 3배로 상향)
...
성능: 2700 파티클(상한)도 `THREE.Points` 단일 draw call 이라 일반 노트북에서 60fps 유지가
어렵지 않다. 평균 alive 수는 spawn rate × 평균 lifetime 으로 ~450~600 개 수준.
